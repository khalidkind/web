...
class TransactionsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $tickets = Transactions::with(['lokasi', 'jenisKendaraan'])->orderBy('id', 'desc')->get();

        return view('transaction.index',['title'=>'transactions'], compact('locations', 'vehicleTypes', 'tickets', 'selectedVehicle'));
    }
    ...
    public function storeEnter(Request $request)
    {
        ...
        $vt = Vehicletypes::find($request->id_jenis);
        ...
        $tx = Transactions::with(['lokasi', 'jenisKendaraan'])->find($ticket->id);
    ...
    public function processExit(Request $request)
    {
        ...
        $transaction = Transactions::where('no_tiket', $request->no_tiket)
            ->whereNull('keluar')
            ->first();
        ...
        $tarif = Vehicletypes::find($transaction->id_jenis);
    ...
    {
        $tx = Transactions::with(['lokasi', 'jenisKendaraan'])->findOrFail($id);
        $pdf = Pdf::loadView('transaction.ticket_pdf', compact('tx'));
